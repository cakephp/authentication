...
* This plugin does intentionally **not** handle authorization. It was [decoupled](https://en.wikipedia.org/wiki/Coupling_(computer_programming)) from authorization on purpose. See also [Computer access control](https://en.wikipedia.org/wiki/Computer_access_control). This plugin handles only with *identification* and *authentication*. We might have another plugin for authorization. 

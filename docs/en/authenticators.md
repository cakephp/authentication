...
The JWT authenticator gets the [JWT token](https://jwt.io/) from the
...
You need to add the lib [firebase/php-jwt](https://github.com/firebase/php-jwt)
...
- **salt**: When `false` no salt is used. When a string is passed that value is used as a salt value.
  ...
  will contain <span class="title-ref">hash(username + password + hmac(username + password, salt))</span>. This helps harden tokens against possible
